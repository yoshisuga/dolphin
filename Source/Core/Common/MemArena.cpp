...
u8* MemArena::FindMemoryBase()
{
...
#elif defined(IOS)
    // FIXME:
    // we have a problem here: our virtual memory address space is severely
    // limited, iPhones only have 1GB of RAM and iOS doesn't have a swap file.
    // here, we're supposed to find 16GB in 64-bit mode, but because that
    // is guaranteed to fail, I set it to only find 1GB. However, I suspect that
    ...
    if (base == MAP_FAILED)
    {
        PanicAlert("Failed to map memory space: %s", strerror(errno));
        return 0;
    }
...
static bool Memory_TryBase(u8 *base, MemoryView *views, int num_views, u32 flags, MemArena *arena)
{
    ...
    for (i = 0; i < num_views; i++)
    {
        ...
        {
            PanicAlert("TryBase: entering if");
            view->view_ptr = views[i - 1].view_ptr;
        }
        else
        {
            PanicAlert("TryBase: entering else");
        ...
        if (!view->view_ptr)
        {
            ...
            PanicAlert("TryBase: error, shutting down");
    ...
    PanicAlert("TryBase: we're done here :D");
    return true;
}
